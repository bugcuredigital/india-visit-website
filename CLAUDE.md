...
5. **Brand tokens only.** Colors: crimson `#C3163A` (CTAs), burgundy `#741238`, deep plum `#452B5E` (dark sections/footer), yellow `#E5C745` (accents — ONLY on plum/burgundy backgrounds, never on off-white), off-white `#F7F5F6` (base). No other colors. Logo: approved artwork only, never redrawn/stretched. Lotus glyph = highlight bullets sitewide (never ✦ or generic checks).
   ...
   **Lotus glyph — recreation authorised.** No vector source exists for the lotus, which is required sitewide as the highlight-bullet mark. The brand's author (BugCure) authorised a faithful vector recreation traced from the brand kit PDF: match the original petal geometry and proportions, never invent a different lotus. Output is single-colour `lotus.svg` using `fill="currentColor"` so one file inherits crimson/plum/white by context. Client approval against a side-by-side comparison is required before it is wired into components.
